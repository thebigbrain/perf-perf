...
const emptyFunction = (...args: any[]): any => {}
const warning = emptyFunction;
...
if (__DEV__) {
  ...
  const updatedAncestorInfo = function(oldInfo: any, tag: any, instance: any) {
   ...
   * Returns whether
   */
  const isTagValidWithParent = function(tag: string, parentTag: string) {
   ...
   * Returns whether
   */
  const findInvalidAncestorForTag = function(tag: string, ancestorInfo: any) {
    switch (tag) {
      case 'address':
      case 'article':
      case 'aside':
  ...
  validateDOMNesting = function(childTag: string, childText: string, ancestorInfo: any) {
  ...
  validateDOMNesting.prototype.updatedAncestorInfo = updatedAncestorInfo;
}
